...
const getAllGenre = (movies) => [...new Set(movies.reduce((prev, elem) => [...prev, ...elem.genre], []))];
console.log(getAllGenre(films));
// Task 2. Собрать в массив всех актеров всех фильмов (без повторения)
const getAllActors = (movies) => [...new Set(movies.reduce((prev, elem) => [...prev, ...elem.actors], []))];
console.log(getAllActors(films));
// Task 3. Отсортировать фильмы по рейтингу по убыванию
const getRatedFilms = (movies) => movies.sort((a, b) => b.imdbRating - a.imdbRating);
console.log(getRatedFilms(films));
const getShortInfoMovies = (movies) => movies.reduce((prev, { id, title, released, plot }) => [...prev, { id, title, released, plot }], []);
...
const getMoviesByYear = (movies, year) => movies.filter((elem) => elem.year === year);
...
const getSearchedMovies = (movies, searchStr) => movies.filter((elem) => elem.title.includes(searchStr));
...
const getSearchedMovies2 = (movies, searchStr) => movies.filter((elem) => elem.title.includes(searchStr) || elem.plot.includes(searchStr));
...
const findFilm = (movies, key, value) => movies.filter((elem) => elem[key] === value);
